...
function create_topics_table() {
    ...
    if ($conn->query($sql) === FALSE) {
        error_log("创建topics表失败: " . $conn->error);
        return false;
    }
    
    // 创建专题页项目表
    $sql = "CREATE TABLE IF NOT EXISTS topic_items (
        id INT AUTO_INCREMENT PRIMARY KEY,
        topic_id INT NOT NULL,
        type ENUM('banner', 'text', 'image', 'link', 'carousel') NOT NULL,
    ...
    if ($conn->query($sql) === FALSE) {
        error_log("创建topic_items表失败: " . $conn->error);
        $conn->close();
        return false;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create_topic'])) {
        ...
        $conn = get_db_connection();
        $sql = "INSERT INTO topics (title, slug, content, status) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $status = 1;
        $content = "<div class='topic-content'></div>";
        $stmt->bind_param("sssi", $title, $slug, $content, $status);
...
if (isset($_FILES['cover_image']) && $_FILES['cover_image']['error'] === 0) {
    ...
    if($check !== false) {
        if (move_uploaded_file($_FILES["cover_image"]["tmp_name"], $target_file)) {
            echo "文件 ". htmlspecialchars(basename($_FILES["cover_image"]["name"])). " 已上传。";
        } else {
            echo "上传文件时出错。";
        }
    } else {
        ...
        <main class="content">
            ...
            <div class="card">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <button type="submit" name="create_topic" class="btn btn-success">创建专题页</button>
                </form>
            </div>
            ...
            <div class="card">
                        ...
                        <tbody>
                            <?php foreach ($topics as $topic): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="../topic/<?php echo $topic['slug']; ?>" target="_blank">
                                            查看专题页
                                        </a>
                                    </td>
                                    <td>
                                        <span class="status-badge status-<?php echo $topic['status'] ? 'active' : 'inactive'; ?>">
                                            <?php echo $topic['status'] ? '已启用' : '已禁用'; ?>
                                        </span>
                                    </td>
                                    <td><?php echo date('Y-m-d H:i', strtotime($topic['created_at'])); ?></td>
                                    <td class="actions">
                                        <a href="edit_topic.php?id=<?php echo $topic['id']; ?>" class="btn">编辑</a>
                                        <form method="POST" style="display: inline-block;">
                                            <input type="hidden" name="id" value="<?php echo $topic['id']; ?>">
                                            <input type="hidden" name="status" value="<?php echo $topic['status'] ? '0' : '1'; ?>">
                                            <button type="submit" name="update_status" class="btn btn-secondary">
                                                <?php echo $topic['status'] ? '禁用' : '启用'; ?>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
                ...
                <?php else: ?>
                    <p style="text-align: center; color: #666; padding: 20px;">暂无专题页数据</p>
                <?php endif; ?>
            </div>
        </main>
    </div>
